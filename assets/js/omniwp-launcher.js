 ...
 *
 *   ?OMNIWP_step=identify   canonical; the only form the server can see
 *   #login                       an alias, resolved to the canonical form
 *   [data-omniwp="<step>"]  for elements that are not links
 *   window.OmniWP.open(step) for themes and other plugins
 *
...
( function () {
	...
	function requestedStep() {
		...
		var queried = resolve( params.get( data.param || 'OMNIWP_step' ) );
	...
	function prefetch( step, redirect ) {
		...
		loadDialog().then( function () {
			if ( window.OmniWPDialogApi && window.OmniWPDialogApi.prefetch ) {
				window.OmniWPDialogApi.prefetch( wanted, redirect || '' );
			}
		} );
	}
	...
	function open( step ) {
		...
		return loadDialog().then( function () {
			if ( window.OmniWPDialogApi ) {
				window.OmniWPDialogApi.open( wanted );
			}
		} );
	}
	...
	/**
	 * Preload assets and step payload on hover before the visitor clicks.
	 */
	function onPointerOver( event ) {
		var trigger = event.target.closest( '[data-omniwp]' );

		if ( trigger ) {
			var asked = resolve( trigger.getAttribute( 'data-omniwp' ) || 'login' );
		...
		if ( ! link || link.closest( '[data-no-omniwp]' ) ) {
			return;
		}
		...
		if ( isCapturedLogin( link ) ) {
			...
			try {
				...
				step = resolve( url.searchParams.get( data.param || 'OMNIWP_step' ) );
				redirect = url.searchParams.get( 'redirect_to' ) || '';
			} catch ( error ) {
				step = '';
			}

			prefetch( step || resolve( 'login' ), redirect );
		}
	}
	...
	function isCapturedLogin( link ) {
		...
		if ( ! captured.length || link.closest( '[data-no-omniwp]' ) ) {
			return false;
		}
	...
	function openFor( link ) {
		...
		return loadDialog().then( function () {
			if ( window.OmniWPDialogApi ) {
				window.OmniWPDialogApi.open( step || resolve( 'login' ), redirect );
			}
		} );
	}
	 ...
	 * `OmniWP_welcome=1`, which is the flag a finished registration has
	 ...
	 * hosting `[OMNIWP]` draws it from the same flag, and opening a dialog
	...
	ready( function () {
		...
		if ( welcome ) {
			...
			loadDialog().then( function () {
				if ( window.OmniWPDialogApi ) {
					window.OmniWPDialogApi.open( welcome );
				}
			} );
