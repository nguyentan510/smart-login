...
( function () {
	...
	function here() {
		...
		url.searchParams.delete( data.param || 'OMNIWP_step' );
		url.hash = '';
	...
	function loadAddressPicker() {
		...
		if ( window.OmniWPAddressEnhance || ! data.addressJs ) {
		...
		window.OmniWPAddress = window.OmniWPAddress || data.address || {};
	...
	function enhanceAddress() {
		...
		loadAddressPicker().then( function () {
			if ( window.OmniWPAddressEnhance ) {
				window.OmniWPAddressEnhance( body );
			}
		} );
	}
	...
	/**
	 * Prefetch a step payload on hover before the visitor clicks.
	 */
	function prefetch( step, redirectTo ) {
		var stepKey = ( step || 'identify' ) + '::' + ( redirectTo || '' );

		if ( prefetched[ stepKey ] ) {
			return prefetched[ stepKey ];
		}

		var url = new URL( data.endpoint );
		url.searchParams.set( 'step', step || 'identify' );
		url.searchParams.set( 'page', here() );
		url.searchParams.set( 'redirect_to', redirectTo || here() );

		prefetched[ stepKey ] = window
	...
	function load( step ) {
		var stepKey = ( step || 'identify' ) + '::' + ( destination || '' );
		var pending = prefetched[ stepKey ];
	 ...
	 * `?OMNIWP_step=identify` and closed it would reopen it on refresh, and
