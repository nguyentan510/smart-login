...
( function () {
	...
	function initContactVerification( root ) {
		root.querySelectorAll( '[data-sl-contact]' ).forEach( function ( panel ) {
			...
			var cancelButton = panel.querySelector( '[data-sl-contact-cancel]' );
			...
			function request( path, payload ) {
				...
				body.OMNIWP_ts = String( data.stamp || '' );
				body.OMNIWP_website = '';
			...
			if ( cancelButton ) {
				cancelButton.addEventListener( 'click', function () {
					message( i18n.contactWait || 'Please wait…' );
					busy( cancelButton, true );

					request( 'contact/cancel', {} )
						.then( function () {
							token = '';
							valueInput.value = '';
							if ( codeInput ) {
							...
							openEditor( false );
							panel.removeAttribute( 'data-sl-contact-pending' );
							document.querySelectorAll( '[data-sl-pending-banner]' ).forEach( function ( banner ) {
								banner.remove();
							} );
							message( i18n.contactCancelled || 'Đã hủy yêu cầu xác thực.', 'success' );
						} )
						.catch( function ( error ) {
							message( error.message, 'error' );
						} )
						.then( function () {
							busy( cancelButton, false );
						} );
				} );
			}
		} );
	...
	function enhance( scope ) {
		( scope || document ).querySelectorAll( '.omniwp' ).forEach( function ( root ) {
