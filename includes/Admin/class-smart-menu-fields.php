 ...
 *
 * Two meta keys, down from five. `_ow_smart_menu_devices`, `_ow_smart_menu_panel`
 * and `_ow_smart_menu_panel_root` were declared here in Phase 25 and left in
 * Phase 26 with the module that read them: NaviKit owns navigation now, and reads
 * its own `_navikit_devices`.
 *
 * Meta a site already stored stays in the database rather than being deleted. It
 * costs a row nobody reads, and removing somebody's saved choices to tidy up
 * after a decision they had no part in is the wrong trade — `uninstall.php` is
 * where data goes when a site asks for it to go.
 *
 * @package OmniWP
 */
