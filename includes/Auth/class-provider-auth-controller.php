...
final class ProviderAuthController {
	...
	public function start(): void {
		...
		if ( ! wp_verify_nonce( (string) wp_unslash( $_GET['_wpnonce'] ?? '' ), 'smart_login_provider_start_' . $provider_id ) ) { // phpcs:ignore WordPress.Security.NonceVerification
		...
		$return_url = wp_validate_redirect( (string) wp_unslash( $_GET['redirect_to'] ?? '' ), '' ); // phpcs:ignore WordPress.Security.NonceVerification
	...
	public function callback(): void {
		...
		$provider = $this->providers->get( $provider_id );
		$state = (string) wp_unslash( $_REQUEST['state'] ?? '' ); // phpcs:ignore WordPress.Security.NonceVerification
		if ( ! $provider || ! $provider->is_available() ) {
			$this->fail( new \WP_Error( 'smart_login_provider_unavailable', __( 'Phương thức đăng nhập chưa sẵn sàng.', 'smart-login' ) ) );
		}
		if ( ! empty( $_REQUEST['error'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
			AuditLog::record( AuditLog::PROVIDER_FAILED, '', array( 'provider' => $provider_id, 'reason' => 'cancelled' ) );
		...
		if ( is_wp_error( $transaction ) ) {
			AuditLog::record( AuditLog::PROVIDER_FAILED, '', array( 'provider' => $provider_id, 'reason' => $transaction->get_error_code() ) );
		...
		if ( is_wp_error( $identity ) ) {
			AuditLog::record( AuditLog::PROVIDER_FAILED, '', array( 'provider' => $provider_id, 'reason' => $identity->get_error_code() ) );
			$this->fail( $identity );
		}
		...
		AuditLog::record( AuditLog::PROVIDER_LOGIN, '', array( 'provider' => $provider_id, 'new_user' => $context->is_new_user ), $result->user_id );
