...
`?smartlogin_welcome=1` (`includes/Frontend/class-shortcodes.php:202-205`), and
...
| --- | --- | --- |
| Pending OTP flow | HttpOnly cookie `smart_login_flow` (`class-pending-session.php:19`) | yes |
| Message after redirect | HMAC-signed cookie `smart_login_flash` (`class-notices.php:16`) | yes |
...
there is `/wp-json/smart-login/v1/…`. Any fragment rendered over REST will emit
...
```php
$requested = isset( $_GET['smart_login_step'] ) ? sanitize_key( wp_unslash( $_GET['smart_login_step'] ) ) : '';
return in_array( $requested, self::PUBLIC_STEPS, true ) ? self::canonical( $requested ) : self::canonical( $fallback );
```
`class-flow.php:78-80`

`?smart_login_step=identify` works today, is already allowlisted, is already
generated by `Flow::url()` (`:124-133`), and is already used by
`ProviderAuthController` as its failure fallback (`:385`).
`?smartlogin_welcome=1` is a second one of the same kind, read in three places.
...
| Full JSON API, templates re-implemented in JavaScript | `assets/js/smart-login.js:1-6` states the plugin's contract: *"Every form works without this file."* A second copy of every template in JS is a second source of truth, and this project has been bitten by drift five times (CLAUDE.md) |
...
| Spelling | Role |
| --- | --- |
| `?smart_login_step=<step>` | **canonical.** The only form the server can see |
| `#login`, `#dang-ky`, … | client alias; the launcher resolves it to the above |
| `data-smart-login="<step>"` | for elements that are not links |
| `window.SmartLogin.open( step )` / `.close()` | for themes and other plugins |
...
A small launcher script (`assets/js/smart-login-launcher.js`) is enqueued
...
- never for `wp-login.php` carrying an `action` (`logout`, `postpass`,
  `resetpass`)
- never for an element marked `data-no-smart-login`

Default on, because a capture that is off by default is a feature nobody
receives. `smart_login_capture_links` turns it off, and it is the same filter a
...
- **No settings screen, and no new tab.** The off switch is the filter
  `smart_login_popup_enabled`, documented in `README.md`. Scope was chosen
